	...
	Elseif PlayerHasBeenToLocationOfBattle && PlayerIsTooFarAway
 		CWScript.Log("CWSiegeScript", self + " CHECKING FOR QUEST UPDATE because PlayerHasBeenToLocationOfBattle && PlayerIsTooFarAway")
			...
			If PlayerIsAttacking
				...
				If GetStageDone(StageThatMeansPlayerIsConsideredNotToHaveRunAwayFromAttack)
					;STAGE IS SET THAT MEANS PLAYER PARTICIPATED ENOUGH:
			...
			elseif PlayerIsAttacking == False
				...
				if GetStageDone(StageThatMeansPlayerIsConsideredNotToHaveRunAwayFromDefense)
					;STAGE IS SET THAT MEANS PLAYER PARTICIPATED ENOUGH:
...
;*** PROCESSING PLAYER RUNNING AWAY	
	if PlayerHasRunAway && !PlayerHasLeftAfterBeingSuccessful
		GoToState("DONE")
		CWScript.Log("CWSiegeScript", self + " calling Stop() because PlayerHasRunAway.")		
		;schofida - don't set this. Even if player ran away, we want Leader to give prize :)
;		LocationOfBattle.SetKeywordData(CWSiegePlayerRanAway, 1)
		if CWS.CWPercentPoolRemainingAttacker.GetValue() == 100.0 && CWS.CWPercentPoolRemainingDefender.GetValue() == 100.0 && PlayerIsAttacking
			CWScript.Log("CWSiegeScript", self + " player ran away before anything happened. Fail attack quest.")	
			SetStage(StageToSetIfPlayerIsDefendingDistanceFail)
		elseif CWS.CWPercentPoolRemainingAttacker.GetValue() == 100.0 && CWS.CWPercentPoolRemainingDefender.GetValue() == 100.0
			CWScript.Log("CWSiegeScript", self + " player ran away before anything happened. Fail defense quest.")
			SetStage(StageToSetIfPlayerIsAttackingDistanceFail)
		elseif CWS.CWPercentPoolRemainingAttacker.GetValue() > CWS.CWPercentPoolRemainingDefender.GetValue()
	...
	elseif PlayerHasLeftAfterBeingSuccessful
		...
		if PlayerIsAttacking && !GetStageDone(StageToSetIfPlayerIsAttackingDistanceFail)
 			CWScript.Log("CWSiegeScript", self + " SetStage("  + StageToSetIfPlayerIsAttackingDistanceFail + ") because PlayerHasRunAway")
			SetStage(StageToSetIfPlayerIsAttackingDistanceFail)
		Elseif  !PlayerIsAttacking && !GetStageDone(StageToSetIfPlayerIsDefendingDistanceFail)
 			CWScript.Log("CWSiegeScript", self + " SetStage("  + StageToSetIfPlayerIsDefendingDistanceFail + ") because PlayerHasRunAway")
			SetStage(StageToSetIfPlayerIsDefendingDistanceFail)
