scriptName CWODisableHavokOnLoadRefAlias extends ReferenceAlias
...
EVENT onCellAttach()
	if (beenSimmed == FALSE && GetRef() != None && GetRef().Is3DLoaded())
		GetRef().setMotionType(GetRef().Motion_Keyframed, TRUE)
...
FUNCTION ReleaseToHavok()
		beenSimmed = TRUE
		objectReference myLink = GetRef().getLinkedRef(linkHavokPartner)
		if myLink != NONE
		...
		GetRef().setMotionType(GetRef().Motion_Dynamic, TRUE)
		GetRef().ApplyHavokImpulse(0, 0, 1, 5)
