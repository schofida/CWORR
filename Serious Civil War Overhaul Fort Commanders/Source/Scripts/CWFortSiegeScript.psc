...
; 	CWScript.Log("CWFortSiegeScript", self + "CreateInteriorDefenders()")
	...
	tryToCreateInteriorCommander(FortLocation, InteriorSpawner1, InteriorDefender1)
...
function tryToCreateInteriorCommander(Location FortLocation, ReferenceAlias InteriorSpawner, ReferenceAlias InteriorDefender)
	ActorBase CommanderActorBase
	ObjectReference InteriorSpawnerRef = InteriorSpawner.GetReference()
	ObjectReference DefenderRef

; 	CWScript.Log("CWFortSiegeScript", self + "tryToCreateInteriorCommander() checking alias " + InteriorSpawner + "and if not empty will create an InteriorCommander at it.")

	
	if CWs.GetDefender(FortLocation) == CWs.iImperials
		CommanderActorBase = CWFortSoldierCommanderImperial
	elseif CWs.GetDefender(FortLocation) == CWs.iSons
		CommanderActorBase = CWFortSoldierCommanderSons
	Else
; 		CWScript.Log("CWFortSiegeScript", self + " error: tryToCreateInteriorCommander() expected 1 or 2 forCWs.GetDefender(FortLocation), instead found:" + CWs.GetDefender(FortLocation), 2, 1, 1)
	EndIf

	if InteriorSpawnerRef
	
		DefenderRef = InteriorSpawnerRef.PlaceActorAtMe(CommanderActorBase)
		InteriorDefender.ForceRefTo(DefenderRef)
	
; 		CWScript.Log("CWFortSiegeScript", self + "tryToCreateInteriorCommander() placed actor " + CommanderRef + " at " + InteriorSpawnerRef + " and forced into alias: " + InteriorDefender)
...
ActorBase Property CWFortSoldierCommanderImperial  Auto  

ActorBase Property CWFortSoldierCommanderSons  Auto  