...
Function Fragment_4()
;BEGIN CODE
; stage 72, repurposed in Balgruuf Dilemma 0.1.4

FavorJarlsMakeFriendsScript favors = Game.GetFormFromFile(0x087e24, "Skyrim.esm") as FavorJarlsMakeFriendsScript

favors.WhiterunSonsGetOutofJail = favors.WhiterunImpGetOutofJail

; don't clear imperial get-out-of-jail
;END CODE
EndFunction
;END FRAGMENT
...
; TODO Idolaf, maybe? (Olfrid is essential in vanilla, but still...)

Actor olfrid = alias_Olfrid.GetActorReference()
olfrid.addToFaction(GovRuling)
olfrid.addToFaction(GovImperial)

; olfrid.addToFaction(JobHousecarlFaction)  ; commented out for fix-08beta

alias_HousecarlImperial.ForceRefTo(olfrid)  ; contains packages for exile, etc ?
...
if getStageDone(30)   ; Hrongar is jarl
  ...
  WhiterunHousecarl_BackupAlias.ForceRefTo(alias_Olfrid.getReference())
endIf
;END CODE
EndFunction
;END FRAGMENT
...
if !getStageDone(30)
  ...
  Actor Olfrid = alias_Olfrid.GetActorReference()
  Olfrid.removeFromFaction(JobHousecarlFaction)
endIf
...
; restart the scene if it was stopped
CW03Scene.Start()
;END CODE
EndFunction
;END FRAGMENT
...
setStage(72)  ; added in Balgruuf Dilemma 0.1.4 - handles thane get-out-of-jails
